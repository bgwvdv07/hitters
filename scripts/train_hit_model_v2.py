...
def resolve_input_path(raw: str) -> Path:
    """Resolve a user-supplied path against cwd first, then fall back to the
    project's output/ dir and project root, so --history-file works no matter
    what directory the script happens to be launched from."""
    p = Path(raw)
    if p.exists():
        return p.resolve()

    candidates = [OUTPUT_DIR / raw, OUTPUT_DIR / Path(raw).name, BASE_DIR / raw]
    for candidate in candidates:
        if candidate.exists():
            return candidate.resolve()

    checked = "\n".join(str(c) for c in [p] + candidates)
    raise FileNotFoundError(f"History file not found: {raw}\nChecked:\n{checked}")
...
def infer_feature_sets(df: pd.DataFrame) -> tuple[list[str], list[str]]:
    excluded = {
        ...
        "walks",
        "strikeouts",
        # prediction/meta columns that should never be model inputs -- if these
        # show up in a history file it means an already-scored or already-graded
        # file got fed back in as raw input somewhere upstream
        "prob_hit_1",
        "prob_hit_2",
        "prob_run_1",
        "pred_hit_probability",
        "model_available_flag",
        "model_rank",
        "source_file",
        "training_row_created_at",
    }
    ...
    # Defensive guard: a column ending in _x/_y means two frames with
    # overlapping non-key columns got merged upstream (e.g. a file that was
    # graded twice). These are corrupt/duplicate data, not real features --
    # never train on them, and warn loudly so the root cause gets fixed.
    suffixed = [c for c in feature_cols if c.endswith("_x") or c.endswith("_y")]
    if suffixed:
        logger.warning(
            "Dropping %s merge-suffixed column(s) from features -- this means "
            "the history file has duplicate/overlapping columns from an "
            "upstream merge collision (e.g. a file graded more than once). "
            "Columns: %s. Consider rebuilding graded_history.csv from clean "
            "source files.",
            len(suffixed),
            suffixed,
        )
        feature_cols = [c for c in feature_cols if c not in suffixed]
...
def main():
    ...
    history_path = resolve_input_path(args.history_file)
